(function() {
  ...
  _.last = function(array, n) {
    return n === undefined ? array[array.length - 1] :
      // Return array n elements from the end
      // If n is greater than the array's length, return the whole array
  ...
  _.each = function(collection, iterator) {
    if (Array.isArray(collection)) {
      ...
      for (const item of collection) {
        iterator(item, index, collection);
        index++;
      }
    } else {
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, (val, i, arr) => {
      // If test passed, push to array
      if (test(val, i, arr)) {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, (value, i, arr) => {
      resultArray.push(iterator(value, i, arr));
    });
  ...
  _.contains = function(collection, target) {
    ...
    let wasFound = function(wasFound, item) {
      if (wasFound) {
        return true;
      }
      return item === target;
